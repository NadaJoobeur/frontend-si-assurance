...
const DevisPage = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((devis) =>
      devis.id.toString().includes(searchTerm.toLowerCase()) ||
      devis.dateCreation.toLowerCase().includes(searchTerm.toLowerCase()) ||
      devis.dateEffet.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, data]);
  ...
  const handleSearch = () => {
    if (filteredData.length === 0 && searchTerm.trim() !== '') {
      toast({
        ...
        description: `Aucun devis trouvé pour "${searchTerm}"`,
        status: "info",
        duration: 3000,
        isClosable: true,
      });
  ...
  return (
    <Box minH="100vh" bg={bgColor} display="flex" _focus={{ outline: "none" }}>
        ...
        <Box flex="1" px={{ base: 4, md: 10 }} py={10} overflowY="auto">
          ...
          <Flex justify="center" mb={8} position="relative">
            <InputGroup maxW="600px">
              <Input
                placeholder="Rechercher un devis par ID ou date..."
                value={searchTerm}
          ...
          {isLoading ? (
            ...
            <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
              {filteredData.map((devisData) => (
                <DevisItem
                  key={`devis-${devisData.id}`}
                  devisData={devisData}
                  onClick={() => navigate(`/devis/detail/${devisData.id}`)}
                  onDelete={handleDelete}
                  onEdit={(id) => navigate(`/devis/edit/${id}`)}
                />
              ))}
            </SimpleGrid>
          ) : (
            <Text textAlign="center" fontSize="lg" mt={10} color="gray.500">
              {searchTerm.trim() === '' ? 'Aucun devis trouvé.' : 'Aucun résultat pour votre recherche.'}
            </Text>
          )}
        </Box>
      ...
      <Tooltip label="Créer un devis" placement="left" hasArrow>
        <Button
